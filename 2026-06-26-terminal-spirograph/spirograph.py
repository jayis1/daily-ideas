...
def compute_curve(curve_type, params, num_points):
    ...
    if curve_type == "hypo":
        ...
        period = 2 * math.pi * r // math.gcd(int(R), int(r)) if r != 0 else 2 * math.pi
        for i in range(num_points):
            t = i / num_points * max(period, 2 * math.pi) * (int(R) // math.gcd(int(R), int(r)) if r != 0 else 1)
            x, y = hypotrochoid_point(R, r, d, t)
            points.append((x, y))
    elif curve_type == "epi":
        R, r, d = params["R"], params["r"], params["d"]
        for i in range(num_points):
            t = i / num_points * 2 * math.pi * 20
            x, y = epitrochoid_point(R, r, d, t)
            points.append((x, y))
    elif curve_type == "rose":
        k, n, d = params["k"], params["n"], params["d"]
        for i in range(num_points):
            t = i / num_points * 2 * math.pi * max(n, 2)
            x, y = rose_point(k, n, d, t)
            points.append((x, y))
    elif curve_type == "lissajous":
        a, b, delta, d = params["a"], params["b"], params["delta"], params["d"]
        for i in range(num_points):
            t = i / num_points * 2 * math.pi * max(a, b, 2)
...
def generate_params(curve_type):
    """Generate random aesthetically pleasing parameters."""
    if curve_type == "hypo":
        R = random.choice([7, 9, 11, 13, 15, 17, 19, 21])
        r = random.choice([2, 3, 4, 5, 6, 7, 8])
        if r >= R:
            r = R - 2
        d = random.choice([r - 1, r, r + 1, r + 2, r * 0.5, r * 1.5])
        d = max(1, abs(d))
        return {"R": R, "r": r, "d": d}
    elif curve_type == "epi":
        R = random.choice([5, 7, 9, 11])
        r = random.choice([2, 3, 4, 5])
        d = random.choice([r - 1, r, r + 1, r * 0.7, r * 1.3])
        d = max(1, abs(d))
        return {"R": R, "r": r, "d": d}
    elif curve_type == "rose":
        ...
        delta = random.choice([0, math.pi / 6, math.pi / 4, math.pi / 3, math.pi / 2])
...
def gallery_mode(width, height, num_points, palette, chars):
...
Examples:
  ...
  %(prog)s --hypo --random --static  Static render of random hypotrochoid
  %(prog)s --hypo --R 21 --r 8 --d 5 --animate --frames 60
        """
    )
    ...
    if args.gallery:
        ...
        gallery_mode(w, h, args.points, args.palette, chars)
        return

    # Build parameters
    params = {}
    if args.random or (curve_type in ("hypo", "epi") and args.R is None):
        params = generate_params(curve_type)
    else:
        if curve_type in ("hypo", "epi"):
            params["R"] = args.R or 11
            params["r"] = args.r or 4
            params["d"] = args.d or (params["r"] or 4)
        elif curve_type == "rose":
            params["k"] = args.k or 5
            params["n"] = args.n or 3
            params["d"] = 10
        elif curve_type == "lissajous":
            params["a"] = args.a or 3
            params["b"] = args.b or 4
    ...
    if args.static or not sys.stdout.isatty():
        static_render(curve_type, params, w, h, args.points, args.palette if sys.stdout.isatty() else "none", chars)
    else:
