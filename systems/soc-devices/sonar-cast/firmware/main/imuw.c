/*
 * sonar-cast / firmware / imuw.c
 * ICM-42688-P IMU read + tilt-from-vertical computation for depth correction.
 *
 * The transducer beam must point straight down for the range-to-depth
 * conversion to be accurate.  We compute the angle between the device's
 * -Z axis (transducer face normal) and the gravity vector.
 */
#include "main.h"
#include <math.h>
...
void imuw_init(void)
{
    uint8_t cfg[2] = { REG_PWR_MGMT0, 0x0C };  /* LN mode accel + gyro */
    ...
    ax_off = sx / 64.0f;
    ay_off = sy / 64.0f;
    az_off = sz / 64.0f - 1.0f;   /* expect +1 g on Z when level, face-down */
}
...
void imuw_read_tilt(float *tilt_deg)
{
    ...
    /* Tilt from vertical (down) = angle between (x,y,z) and (0,0,-1).
       cos(θ) = -z / |(x,y,z)| */
