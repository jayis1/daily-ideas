/*
 * lode-sweep / esp32c3 / main.c
 * ESP32-C3 radio/GPS relay firmware (ESP-IDF v5.2+).
 *
 * Receives detection results from the STM32 over UART0, parses GPS NMEA from
 * a NEO-M9N on UART1, and:
 *   - Exposes a BLE GATT server (target results stream)
 *   - Serves a Wi-Fi AP web dashboard (leaflet.js survey map)
 */
#include <stdio.h>
#include <string.h>
...
static const char *TAG = "lode-sweep-radio";
...
/* Target class names (must match firmware/main/main.h) */
    ...
    while (1) {
        ...
        if (n > 0) {
            /* Forward to BLE / Wi-Fi in a real build.
               Parse the result frame and extract target class. */
            if (n >= 10 && buf[0]==0xAA && buf[1]==0x55) {
                uint16_t body_len = buf[2] | (buf[3] << 8);
                if (body_len >= 2 && buf[4] == 0x01) {
                    uint8_t target_class = buf[5];
                    float depth_cm;
                    memcpy(&depth_cm, &buf[6 + 4 + 4], 4); /* skip conf+depth */
                    ESP_LOGI(TAG, "Target: %s, depth: %.1f cm",
                             target_class < 8 ? CLASS_NAMES[target_class] : "?",
                             depth_cm);
                }
            }
        }
...
void app_main(void)
{
    ESP_LOGI(TAG, "Lode Sweep radio/GPS booting...");
    nvs_flash_init();
